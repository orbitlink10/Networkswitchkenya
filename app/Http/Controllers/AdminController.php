...
class AdminController extends Controller
{
     ...
     * @return array<string, mixed>
     */
    private function switchAttributeRules(): array
    {
        return [
            'port_count' => ['nullable', 'integer', 'min:0', 'max:255'],
            'rj45_ports' => ['nullable', 'integer', 'min:0', 'max:255'],
            'sfp_ports' => ['nullable', 'integer', 'min:0', 'max:255'],
            'sfp_plus_ports' => ['nullable', 'integer', 'min:0', 'max:255'],
            'sfp28_ports' => ['nullable', 'integer', 'min:0', 'max:255'],
            'qsfp_ports' => ['nullable', 'integer', 'min:0', 'max:255'],
            'poe_ports' => ['nullable', 'integer', 'min:0', 'max:255'],
            'poe_budget' => ['nullable', 'integer', 'min:0'],
            'poe_max_per_port' => ['nullable', 'integer', 'min:0'],
            'poe_standard' => ['nullable', Rule::in(['af', 'at', 'bt'])],
            'management_type' => ['nullable', Rule::in(array_keys(SwitchCatalog::managementTypes()))],
            'layer' => ['nullable', Rule::in(['l2', 'l3'])],
            'port_speed' => ['nullable', Rule::in(array_keys(SwitchCatalog::speeds()))],
            'uplink_type' => ['nullable', Rule::in(array_keys(SwitchCatalog::uplinks()))],
            'cooling' => ['nullable', Rule::in(['fanless', 'active'])],
            'availability' => ['nullable', Rule::in(['in_stock', 'out_of_stock', 'preorder'])],
            'switching_capacity' => ['nullable', 'string', 'max:60'],
            'forwarding_rate' => ['nullable', 'string', 'max:60'],
            'mac_table' => ['nullable', 'string', 'max:40'],
            'power_supply' => ['nullable', 'string', 'max:120'],
            'power_consumption' => ['nullable', 'string', 'max:60'],
            ...
            'lacp' => ['nullable', 'boolean'],
            'snmp' => ['nullable', 'boolean'],
            'acl' => ['nullable', 'boolean'],
            'rackmount' => ['nullable', 'boolean'],
            'desktop' => ['nullable', 'boolean'],
            'din_rail' => ['nullable', 'boolean'],
            'outdoor' => ['nullable', 'boolean'],
            'industrial' => ['nullable', 'boolean'],
            'stackable' => ['nullable', 'boolean'],
            'applications' => ['nullable', 'array'],
            'applications.*' => ['string', Rule::in(array_keys(SwitchCatalog::applications()))],
        ];
    }
     ...
     * @return array<string, mixed>
     */
    private function switchAttributePayload(array $data, Request $request): array
    {
        $payload = [];

        $numeric = [
            'port_count', 'rj45_ports', 'sfp_ports', 'sfp_plus_ports', 'sfp28_ports',
            'qsfp_ports', 'poe_ports', 'poe_budget', 'poe_max_per_port',
        ];
        foreach ($numeric as $field) {
            $payload[$field] = ($data[$field] ?? null) !== null && ($data[$field] ?? null) !== ''
                ? (int) $data[$field]
                : null;
        }
        ...
        foreach ($stringLimits as $field => $limit) {
            $payload[$field] = $this->cleanOptionalText($data[$field] ?? null, $limit);
        }

        $booleans = [
            'vlan_support', 'qos', 'stp', 'lacp', 'snmp', 'acl', 'rackmount',
            'desktop', 'din_rail', 'outdoor', 'industrial', 'stackable',
        ];
        ...
        $applications = collect($data['applications'] ?? [])
            ...
            ->values()
            ->all();
        $payload['applications'] = $applications !== [] ? $applications : null;
    ...
    private function adminVendor(Request $request, bool $create = false): ?Vendor
    {
        ...
        $adminShopName = 'Network Switches Kenya Store';
    ...
    public function storeCategory(Request $request): RedirectResponse
    {
        ...
        $rules = [
            ...
            'import_url' => ['nullable', 'url', 'max:500'],
        ];
    ...
    public function storeProduct(Request $request): RedirectResponse
    {
        ...
        if (Product::switchFieldsReady()) {
            $rules = array_merge($rules, $this->switchAttributeRules());
        }
        ...
        if (Product::switchFieldsReady()) {
            $payload = array_merge($payload, $this->switchAttributePayload($data, $request));
        }
        ...
        if ($importUrl = $request->input('import_url')) {
            \App\Support\ImageManager::import($product, $importUrl, $product->images()->count() === 0);
        }
